    ...
    const fetchData = async () => {
      ...
      if (session?.user) {
        const { data } = await supabase
          .from("profiles")
          .select("is_admin, user_stats (points, current_streak)")
        ...
        <div className="p-4 border-t border-white/10 space-y-3">
          ...
          {isAdmin && (
            <Link
              ...
              className="w-full flex items-center justify-center gap-2 py-2 text-violet-300 hover:text-white hover:bg-white/10 rounded-xl transition-all text-xs font-bold uppercase tracking-widest"
            >
