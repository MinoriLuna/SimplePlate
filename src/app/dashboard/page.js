...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      const { data: mealsData } = await supabase
        .from("meals")
        .select("id, dish_name, meal_type, logged_at")
  ...
  const handleDeleteMeal = async (mealId) => {
    // 1. Remove from Supabase meals table
    ...
    if (error) {
      ...
      // 2. Update UI state instantly to remove the card
          ...
          <div className="lg:col-span-7 bg-white p-8 rounded-3xl shadow-sm border border-slate-100 flex flex-col h-full">
            ...
            <div className="space-y-4 flex-grow">
              {todayMeals.length === 0 ? (
                ...
                todayMeals.map((meal) => (
                  <div key={meal.id} className="group flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100 hover:border-red-100 transition-all">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center justify-center text-center min-h-[200px]">
                ...
                <h3 className="text-3xl font-extrabold text-green-500 tracking-tight">Amazing!</h3>
