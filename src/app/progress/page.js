...
return (
  <div className="relative min-h-screen font-sans text-slate-800 no-scrollbar">
    ...
    {/* LOADING GUARD */}
    {isLoading || !profile ? (
      <div className="fixed inset-0 z-[100] bg-[#F2FFF5] flex flex-col items-center justify-center">
          ...
          <div className="lg:col-span-5 space-y-6">
                ...
                <div className="flex justify-between items-center mb-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2"> 
                      ...
                      {hasBadge && (
                        <img src="/images/goldenbadge.png" alt="Golden Badge" className="w-8 h-8 sm:w-10 sm:h-10 object-contain" />
                      )}
                    </div>
                  </div>
            ...
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <div className={`w-8 h-8 rounded-full flex items-center justify-center bg-green-50 text-green-600 transition-transform ${showDetails ? 'rotate-180' : ''}`}>
                ...
                {nutrientBalance.map((stat, index) => (
                  <div key={index}>
                    ...
                    <div className="w-full h-2.5 bg-slate-50 rounded-full overflow-hidden border border-slate-100">
                      <div className={`h-full ${stat.col} transition-all duration-1000`} style={{ width: `${stat.pct}%` }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="lg:col-span-7">
            <div className="bg-white/80 backdrop-blur-sm rounded-[2rem] p-8 lg:p-10 shadow-xl border border-green-200 h-full">
              ...
              <AnimatePresence mode="wait">
                {!aiInsight ? (
                  <motion.div key="gen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-12 text-center border-2 border-dashed border-green-100 rounded-[2rem] bg-[#F2FFF5]/30">
                    ...
                    <div className="p-6 bg-[#F2FFF5]/80 rounded-[2rem] border border-green-200 flex gap-4">
                      ...
                      <div>
                        ...
                        <div className="mt-4 p-3 bg-white/50 rounded-xl inline-block border border-green-100/50">
                           <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Goal: {aiInsight.improvement}</p>
                        </div>
                      </div>
                    </div>
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-[2.5rem] p-10 shadow-sm border border-green-200 max-w-3xl align-center mx-auto overflow-y-auto custom-scrollbar max-h-[600px]">
                ...
                <div className="space-y-4">
                  {mealsData.map((meal, i) => {
                    ...
                    return (
                      <div key={i} className={`overflow-hidden transition-all duration-300 border rounded-2xl ${isExpanded ? 'bg-white border-green-400 shadow-md' : 'bg-[#F2FFF5]/30 border-green-200'}`}>
                        <button onClick={() => setExpandedMealId(isExpanded ? null : i)} className="w-full flex flex-col md:flex-row md:items-center justify-between p-5 gap-4 text-left">
                          ...
                          <div className="flex items-center justify-between md:justify-end gap-6">
                            <p className="text-[10px] font-bold text-slate-400 uppercase whitespace-nowrap">{new Date(meal.logged_at).toLocaleDateString('en-MY', { day: 'numeric', weekday: 'long' })}</p>
                            <div className={`text-slate-300 transition-transform ${isExpanded ? 'rotate-180' : ''}`}><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7"></path></svg></div>
                          </div>
                        </button>
                        <AnimatePresence>
                          {isExpanded && (
                            <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="px-6 pb-6 border-t border-green-50 bg-slate-50/50 overflow-hidden ">
                              <div className="pt-5 grid grid-cols-4 gap-4">
                                ...
                                <div className="text-center border-r border-slate-200"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Fat</p><p className="text-sm font-black text-orange-500">{meal.fat_g}g</p></div>
                                <div className="text-center"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Vitamins</p><p className="text-sm font-black text-green-600">{meal.vitamins}mg</p></div>
                              </div>
                            </motion.div>
                          )}
                        </AnimatePresence>
